...
Do you want a way to back up your Jellyfin libraries' images?
...
In addition, you can download embedded versions of the generated HTML files to easily back up your Jellyfin libraries' images.
...
- It is not advised to expose this service publicly.
...
2. On the main page:
	...
	- Pick colors and image types
	- View or download "embedded" versions of previously generated HTMLs
3. Hit **Generate**:
	- A background thread calls `generate_html.py`, which talks to Jellyfin’s API, fetches all your items, and checks every selected image type
	- A timestamped `.html` file gets saved under `output/<LibraryName>/`
	- View the HTML in your browser or download an “embedded” version where all images are base64-encoded (for sharing/archiving)
4. Browse results:
    - View the table summary
	- Click on a title in the table to jump to the entry
	- View the gallery
	- Click on an entry's title to jump directly into Jellyfin so you can edit/add images
5. Lightbox:
    - Use the `Prev`, and `Next` buttons or the `←` and `→` keys on your keyboard to navigate between an entry's images
	- Use the `Close` button, the `Esc` key on your keyboard, click on the image, or click outside the image to exit the lightbox
